...
export function connectRoutes(app: Fastify) {
    ...
    app.get('/v1/connect/github/callback', {
        ...
        if (!tokenData) {
            ...
            return reply.redirect('https://app.vibe-on-the-go.com?error=invalid_state');
        }
        ...
        if (!clientId || !clientSecret) {
            return reply.redirect('https://app.vibe-on-the-go.com?error=server_config');
        }
        ...
        try {
            ...
            if (tokenResponseData.error) {
                return reply.redirect(`https://app.vibe-on-the-go.com?error=${encodeURIComponent(tokenResponseData.error)}`);
            }
            ...
            if (!userResponse.ok) {
                return reply.redirect('https://app.vibe-on-the-go.com?error=github_user_fetch_failed');
            }
            ...
            return reply.redirect(`https://app.vibe-on-the-go.com?github=connected&user=${encodeURIComponent(userData.login)}`);

        } catch (error) {
            log({ module: 'github-oauth' }, `Error in GitHub GET callback: ${error}`);
            return reply.redirect('https://app.vibe-on-the-go.com?error=server_error');
        }
    });
